...
void setup_paging() {
	...
	page_table_t* page_dir = (page_table_t*)kmalloc_a(1024 * sizeof(page_table_t));

	memset(page_dir, 0, 1024 * sizeof(page_table_t));
	memset(page_table_identity, 0, 1024 * sizeof(page_t));
	...
	for (size_t i = 0; i < 1024; ++i) {
		...
		page_table_identity[i].s.sup  = 1;
		page_table_identity[i].s.addr = addr_shift(i * 0x1000);
	}

	flush_paging((uint32_t)page_dir);
}
